...
export class Signatures extends Testing.Case {
  ...
  /**
   * Load signature successful.
   */
  @Testing.Method()
  @Class.Public()
  public async loadSignatureSuccessful(): Promise<void> {
    // Create new signature.
    const inserted = <SignRequest.Signatures.Entity>await Helper.getNewSignatureEntity();
    this.areNotSame(inserted, void 0);
    // Test signature loading.
    const loaded = <SignRequest.Signatures.Entity>await this.signatures.load(inserted.uuid);
