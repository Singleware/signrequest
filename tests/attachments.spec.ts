...
export class Attachments extends Testing.Case {
  ...
  /**
   * Load attachment successful.
   */
  @Testing.Method()
  @Class.Public()
  public async loadAttachmentSuccessful(): Promise<void> {
    // Create new attachment.
    const inserted = <SignRequest.Attachments.Entity>await Helper.getNewAttachmentEntity();
    this.areNotSame(inserted, void 0);
    // Test attachment loading.
    const loaded = <SignRequest.Attachments.Entity>await this.attachments.load(inserted.uuid);
